...
void build_sin_trajectory(double stepping, double total_time, double amplitude, double cycles, robmovil_msgs::Trajectory& trajectory_msg, nav_msgs::Path& path_msg)
{
  ...
  for (double t = 0; t <= 1; t = t + stepping)
  {
    point_msg.transform.translation.x = 0;
    point_msg.transform.translation.y = 0;
    point_msg.transform.translation.z = 0;
    point_msg.transform.rotation = tf::createQuaternionMsgFromYaw( t*M_PI/4 );
    trajectory_msg.points.push_back( point_msg );

    stamped_pose_msg.pose.position.x = 0;
    stamped_pose_msg.pose.position.y = 0;
    stamped_pose_msg.pose.position.z = 0;
    
    stamped_pose_msg.pose.orientation = tf::createQuaternionMsgFromYaw(t*M_PI/4);
  ...
  // stamped_pose_msg.pose.orientation = tf::createQuaternionMsgFromYaw(M_PI/4);
